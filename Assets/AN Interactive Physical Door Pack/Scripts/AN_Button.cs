...
public class AN_Button : MonoBehaviour
{
    ...
    public void Press()
    {
        ...
        {
            DoorObject.Action();  // 开/关门

            if (isLever)
            {
                if (anim) anim.SetBool("LeverUp", DoorObject.isOpened);
            }
            else
            {
                if (anim) anim.SetTrigger("ButtonPress");
            }
        }
    }
